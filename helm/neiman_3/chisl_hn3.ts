...
function f(){
	for(let count=0; count<1000; count++){
		for(let j=1; j<Ny; j++){
			for(let i=1; i<Nx; i++){
				// let n = ( 4*(arrU[j][i+1] + arrU[j][i-1]  + arrU[j+1][i] + arrU[j-1][i]) + 
				// 		(arrU[j+1][i+1] + arrU[j-1][i-1] + arrU[j-1][i+1] + arrU[j+1][i-1])  - 6*h*h*Fij(0+i*hx, 0+j*hy)) 
				// 		/ (20 - 6*h*h*kvadrat_k)


				let n = ( ( arrU[j][i+1] + arrU[j][i-1] ) / (hx * hx) + (arrU[j+1][i] + arrU[j-1][i] ) / (hy * hy) - Fij(0+i*hx, 0+j*hy)) 
				/ ((2/(hx*hx)) + (2/(hy*hy)) - kvadrat_k)
