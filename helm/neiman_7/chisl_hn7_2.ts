...
function Fij(x: number, y: number) {
	return 2*Math.exp(y)*Math.sin(PI*x);
}
...
function gran(x: number, y: number){
	return PI*y*Math.exp(y);
}


function gran_R(x: number, y: number){
	return -1*PI*y*Math.exp(y);
}
...
function f(){
	for(let count=0; count<60000; count++){
		...
		for(let j=0; j<=Ny; j++){
			newArrU[j][Nx] = newArrU[j][Nx-1] + gran_R(1,0+j*hy) * hx;
		}
...
function reshenie(x: number, y: number) {
	return y*Math.exp(y)*Math.sin(PI*x);
}
